

#include "as5048a.h"
#include "spi.h"


void AS5048A_Config_CS(void *handle){
	Encoder_t *encd = (Encoder_t *)handle; // Ép kiểu
	GPIO_InitTypeDef as5048a = {0};
	as5048a.Pin = encd->active_port->cs_pin;
	as5048a.Mode = GPIO_MODE_OUTPUT_PP;
	as5048a.Pull = GPIO_NOPULL;
	as5048a.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
	HAL_GPIO_Init(encd->active_port->cs_port, &as5048a);
}
//

static uint8_t calc_even_parity(uint16_t value){
	value ^= value >> 8;
	value ^= value >> 4;
	value ^= value >> 2;
	value ^= value >> 1;
	return value & 1;
}

//

// int AS5048A_Config_ALL(AS5048A_t *encd){
// 	if (encd == NULL) {
// 		return 0;
// 	}
// 	AS5048A_Config_CS(encd);
// 	AS5048A_cs_set(encd);
	
// 	return 1;
// }

int AS5048A_CheckExist(void *handle) {
	Encoder_t *encd = (Encoder_t *)handle;
	uint16_t cmd = AS5048A_WRITE_CMD | AS5048A_REG_ANGLE;
	cmd |= (calc_even_parity(cmd) << 15);

	AS5048A_cs_reset(encd);
	HAL_SPI_TransmitReceive(encd->active_port->hspi, (uint8_t*)&cmd, encd->spi_rx_buffer, 2, 100);
	AS5048A_cs_set(encd);
	HAL_Delay(1);
	AS5048A_cs_reset(encd);
	HAL_SPI_TransmitReceive(encd->active_port->hspi, (uint8_t*)&cmd, encd->spi_rx_buffer, 2, 100);
	AS5048A_cs_set(encd);

	const uint16_t raw_data = ((uint16_t)encd->spi_rx_buffer[0] << 8) | encd->spi_rx_buffer[1];
	if (raw_data == 0x0000 || raw_data == 0xFFFF) return -1; // No Device

	uint16_t data_15bit = raw_data & 0x7FFF;
	if (calc_even_parity(data_15bit) != ((raw_data >> 15) & 0x1)) return 0; // Have Device but error
	
	// Error flag check (bit 14)
	// if ((raw_data >> 14) & 0x1) {
	// 	return 0;
	// }

	return 1;//Have Device
}
//

int AS5048A_DetectExit(void *handle){
	int8_t Count = 0;
	for(uint8_t i = 0; i < 10; i++){
		Count += AS5048A_CheckExist(handle);
		HAL_Delay(1);
	}
	if (Count > 4) return 1; 
	return 0;
}
//

int AS5048A_StartRead(void *handle) {
	Encoder_t *self = (Encoder_t *)handle;
	uint16_t cmd = AS5048A_WRITE_CMD | AS5048A_REG_ANGLE;
	cmd |= (calc_even_parity(cmd) << 15); 

	AS5048A_cs_reset(self);
	if (HAL_SPI_TransmitReceive_DMA(self->active_port->hspi, (uint8_t*)&cmd, self->spi_rx_buffer, 2) != HAL_OK) {
		return 0;
	}
	return 1;
}
//
float AS5048A_ParseData(void *handle) {
	Encoder_t *self = (Encoder_t *)handle;
	AS5048A_cs_set(self);
	
	const uint16_t raw_data = ((uint16_t)self->spi_rx_buffer[0] << 8) | self->spi_rx_buffer[1];
	const uint16_t data_15bit = raw_data & 0x7FFF;
	
	if (raw_data == 0x0000 || raw_data == 0xFFFF) return -1; // No Device
	if (calc_even_parity(data_15bit) != ((raw_data >> 15) & 0x1)) return -1.0f;
	
	// Error flag check (bit 14)
	// if ((raw_data >> 14) & 0x1) {
	// 	return -1.0f;
	// }
	return (float)(raw_data & 0x3FFF) * ANGLE_SCALE_FACTOR;
}
//








